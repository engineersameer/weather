...
let totalPages = Math.ceil(forecastData.length / rowsPerPage);

// Update the table when the page changes
...
// Modify fetchWeatherData to call updateTable after setting forecastData
function displayForecastTable(data) {
    forecastData = data.list.map(item => ({
        date: new Date(item.dt_txt).toLocaleDateString(),
        temperature: item.main.temp,
    ...
    totalPages = Math.ceil(forecastData.length / rowsPerPage); // Calculate total pages
...
function handleChatQuery(query) {
    ...
    if (weatherRegex.test(query)) {
        ...
        const city = cityMatch ? cityMatch[2].trim() : "Islamabad"; // Default to "London" if no city is mentioned
